...
function updateAI(ent, dt) {
  ...
  if (target && ent.ai.retarget <= 0) {
    ...
    ent.ai.retarget = rand(.3, .7);
  }
  if (target) {
    const toward = angleTo(ent.x, ent.y, target.x, target.y);
    const sight = entityCanSee(ent, target);
    ent.angle = toward;
    if (distance > 300 || !sight) {
      dx = Math.cos(ent.ai.wander);
      dy = Math.sin(ent.ai.wander);
    } else if (distance < 170) {
      dx = Math.cos(toward + Math.PI + ent.ai.strafeDir * 0.6);
      dy = Math.sin(toward + Math.PI + ent.ai.strafeDir * 0.6);
    } else {
      dx = Math.cos(toward + ent.ai.strafeDir * 1.2);
      dy = Math.sin(toward + ent.ai.strafeDir * 1.2);
    }
    if (ent.shootCd <= 0 && sight && distance < 420 && ent.ammo > 0) {
      createProjectile(ent, toward + rand(-0.12, 0.12));
      ent.shootCd = rand(.45, .75);
      ent.ammo -= 1;
...
function render() {
  const cam = {
    x: clamp(player.x - WORLD_W * 0.25, 0, WORLD_W - canvas.width),
    y: clamp(player.y - WORLD_H * 0.25, 0, WORLD_H - canvas.height)
  };
  drawMap(cam);
  drawProjectiles(cam);
